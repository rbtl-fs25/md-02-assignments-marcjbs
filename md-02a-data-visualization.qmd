...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.\
    \
    Link to solution:** <https://www.perplexity.ai/search/how-to-ignore-outliers-in-r-Cp7pVecySXWvitg4Fmq5rQ>
...
5.  What are the data types of the three variables used for aesthetic mappings?\
    ...
    -   lifeExp: numerical continuous variables\
    -   What is the third variable? (for gapminder_2007: dataset, for NA: a "None" value..)?
...
```{r}
ggplot(
  data = gapminder_summary_continent_year,
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:\
    The "geom_bar()" function counts any occurring x axis values (in this case continents) and adds them up at the y-axis.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.\
    \
    The "geom_histogram()" function also just counts the occurring values of the given x variable in the dataset, similarly to "geom_bar()".
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:\
    \
    The 'breaks' argument gives the range in which the x-value is displayed in the plot. The first number represents the start point, the second number the end point and the third number the width of each box in the histogram.
...
ggplot(
  data = gapminder_2007,
  mapping = aes(x = gdpPercap, y = lifeExp, size = pop, facet_wrap(~continent), color = country)
) + 
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.\
    \
    For each country in the dataset a specifc color is saved in this object.
